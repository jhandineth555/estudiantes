...
import { downloadPdf, getFotoPerfil, uploadFotoPerfil } from '../../services/Service';
...
const CuScreen = () => {
  ...
  // Ocultar o mostrar el tab según estado del teclado
  useEffect(() => {
  ...
  // Scroll para ocultar tabBar (opcional)
  ...
  // Cargar foto desde backend al iniciar
  useEffect(() => {
    const cargarFotoPerfil = async () => {
      try {
        const base64 = await getFotoPerfil();
        if (base64) {
  ...
  // Elegir imagen y subirla al backend
    ...
    launchImageLibrary(
      ...
      async (response) => {
        if (response.assets && response.assets.length > 0) {
          ...
          try {
            await uploadFotoPerfil(base64Img);
            await AsyncStorage.setItem('profileImage', dataUri);
